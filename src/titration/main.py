...
def titrageConductimetrique(title, x, y, showVeLine: bool = False):
    """
    ...
    # Original point
    plt.scatter(x[1], y[1], marker="o", color="chocolate", label="Experimental points")
    ...
    ax.plot(
        ...
        label=f"Linear regression 1 ($R^2$ = {round(r1**2, 4)})",
    )
    ax.plot(
        x2_new,
        m2 * x2_new + p2,
        color="mediumslateblue",
        label=f"Linear regression 2 ($R^2$ = {round(r2**2, 4)})",
    )
